...
pkg-config = '/opt/cross/aarch64-linux-musl/bin/aarch64-linux-musl-pkg-config'
...
EOF
    ...
    tar -xf "$tmpdir/linux-headers.apk" -C "$BUILDENV_DIR/opt/cross/aarch64-linux-musl" 2> >(grep -v "APK-TOOLS.checksum.SHA1" >&2)
    rm -rf "$tmpdir"
}